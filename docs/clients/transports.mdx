...
STDIO transports do not maintain sessions across multiple client contexts unless `keep_alive=True` is passed to the client. This improves performance by reusing the same subprocess for multiple connections, but can be controlled when you need isolation.
...
client = Client(transport, keep_alive=True)
