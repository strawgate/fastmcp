...
<ParamField body="enabled" type="bool" default="True">
  ...
  A boolean to enable or disable the prompt. See [Visibility Control](#visibility-control) for the recommended approach.
</ParamField>
...
You can control which prompts are visible to clients using server-level visibility control. Disabled prompts don't appear in `list_prompts` and can't be called.
