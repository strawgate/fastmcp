...
<ParamField body="enabled" type="bool" default="True">
  ...
  A boolean to enable or disable the resource. See [Visibility Control](#visibility-control) for the recommended approach.
</ParamField>
...
You can control which resources are visible to clients using server-level visibility control. Disabled resources don't appear in `list_resources` and can't be read.
